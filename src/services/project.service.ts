...
export const generateProjectIdeas = async (
  category: string,
  idea: string,
  type_of_members: string
) => {
  try {
    const prompt = `
            ---IDEA--- : ${idea}
            I want you to generate 3 well written project ideas under the ${category} category
            ${
              idea &&
            ...
            Based on the project, generate an attribute "core_skills" that'll be an array of 3 core skills in the ${category} space needed for the project. I am looking for members with ${type_of_members}
            Based on the project, generate an attribute "interest" that'll be an array of 2 interests to look out for in a user I want to invite. I am looking for members with ${type_of_members}
